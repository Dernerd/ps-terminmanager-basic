...
function appointment_convert_minutes_to_human_format( $duration ) {
	if ( 60 > $duration ) {
		return sprintf( _x( '%d Minuten', 'weniger als 60 Minuten', 'appointments' ), $duration );
	}
	$hours = floor( $duration / 60 );
	$text = sprintf(
		_nx( '%d Stunde', '%d Stunden', $hours, 'mehr als 60 Minuten, hours', 'appointments' ),
		$hours
	);
	...
	if ( 0 < $minutes ) {
		$text .= ' ';
		$text .= sprintf(
			_nx( '%d Minute', '%d Minuten', $minutes, 'mehr als 60 Minuten, minutes', 'appointments' ),
			$minutes
		);
	}
	return $text;
