...
class Appointments_Appointments_Google_Calendar {
	...
	public function __construct() {
		...
		if ( ! defined( 'APP_GCAL_MAX_RESULTS_LIMIT' ) ) {
			define( 'APP_GCAL_MAX_RESULTS_LIMIT', 500, );
		}
	...
	public function add_appointments_hooks() {
		add_action( 'psource_appointments_insert_appointment', array( $this, 'on_insert_appointment' ), 200 );
		add_action( 'psource_appointments_update_appointment', array( $this, 'on_update_appointment' ), 200, 3 );
		add_action( 'appointments_delete_appointment', array( $this, 'on_delete_appointment' ) );
	}

	public function remove_appointments_hooks() {
		remove_action( 'psource_appointments_insert_appointment', array( $this, 'on_insert_appointment' ), 200 );
		remove_action( 'psource_appointments_update_appointment', array( $this, 'on_update_appointment' ), 200 );
	...
	public function edit_inline_gcal_fields( $deprecated, $app ) {
		...
		if ( $show ) {
			$html .= '<label class="title">'.__( 'Google Kalender Beschreibung', 'appointments' );
	...
	public function import() {
		...
		wp_send_json( array( 'message' => sprintf( __( '%d aktualisiert, %d neu eingefügt und %d gelöscht', 'appointments' ), $results['updated'], $results['inserted'], $results['deleted'] ) ) );
	}
	...
	public function get_summary() {
		if ( empty( $this->summary ) ) {
			$this->summary = __('SERVICE Termin','appointments');
		}
		return $this->summary;
	}
